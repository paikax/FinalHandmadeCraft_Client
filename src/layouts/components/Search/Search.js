...
function Search({ id }) {
    ...
    return (
        <div className="relative flex items-center max-sm:hidden">
            <span className="absolute top-0 left-0 bottom-0 flex items-center justify-center px-[8px] cursor-pointer text-black hover:bg-[rgba(0,0,0,0.05)] transition-colors">
                <SearchIcon />
            </span>
            <input
                type="text"
                ...
                spellCheck={false}
                placeholder="Search"
                className="flex-1 py-[8px] px-[40px] h-[38px] bg-gray-200 rounded-md text-xl"
                onChange={handleChange}
                onFocus={() => setShowResult(true)}
            />
            {searchValue && (
                <span
                    className="absolute top-0 bottom-0 right-0 flex items-center justify-center px-[8px] cursor-pointer text-[#4a8f92] hover:bg-[rgba(0,0,0,0.05)] transition-colors"
                    onClick={handleClear}
                >
                    <ClearIcon />
                </span>
